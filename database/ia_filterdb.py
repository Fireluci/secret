...
async def get_search_results(chat_id, query, file_type=None, max_results=10, offset=0, filter=False, **kwargs):
    ...
    # 1. Strict Query: All words must match completely as distinct words (\b)
    ...
    # 2. Pure Inside-Word Fuzzy Query: Substring containment matching (e.g. "throne" in "thrones")
    ...
    # Combine results: Strict matches first, followed by inside-word fuzzy matches (removing duplicates)
    seen_ids = set()
    combined_files = []
    
    for file in strict_files:
        file_id = file.file_id if hasattr(file, 'file_id') else file.get('file_id') if isinstance(file, dict) else str(file._id)
    ...
    # Paginate the combined, prioritized list
