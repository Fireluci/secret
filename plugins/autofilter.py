...
from info import *
from utils import connected_group, get_settings, get_size, is_group_connected, is_subscribed, search_gagala, temp
...
async def cb_handler(client, query):
    if query.data.startswith("spellclose#"):
        try:
            ...
            logger.exception("Spellcheck close button error")
            return await query.answer(ALRT_TXT, show_alert=True)
    ...
    if query.data == "close_data":
        try:
            ...
            logger.exception("Close button error")
        return await query.answer("Closed!")
...
async def advantage_spell_chok(client, msg):
    ...
    btn.append([InlineKeyboardButton("×××× ⟨ Close ⟩ ××××", callback_data=f"spellclose#{reqstr1}")])
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        return await message.reply_photo(photo=PICS, caption=START_TXT, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💎 Buy Premium", callback_data="buy_premium_start")]]), parse_mode=enums.ParseMode.HTML)    
