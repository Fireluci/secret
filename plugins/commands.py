                ...
                try:
                    async for user_doc in col.find({"active": True}):
                        ...
                        if now >= expires_at:
                            ...
                            await log_premium_action(client, f"<b>❌ Premium Expired & Ejected</b>\n\n👤 <a href='tg://user?id={user_id}'>{u_name}</a> (<code>{user_id}</code>)\n⌛ Expired: {exp_str}")
                            try:
                    ...
                    logger.error(f"DB error in expiry loop: {db_err}")
        except Exception as e:
...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        return await client.send_message(
            ...
            "<b>🔆 First Join Our Main Channel & Then Click Try Again ♻</b>", 
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🏮 Main Channel", url=FORCE)], 
                [InlineKeyboardButton("🔄 Try Again", url=f"https://telegram.me/{temp.U_NAME}?start={payload}")]
            ]), 
            parse_mode=enums.ParseMode.HTML
        )
    if len(message.command) == 2 and message.command[1] == "i_paid":
        MINIMAL_PENDING_FLOW[message.from_user.id] = {"status": "waiting_screenshot"}
        return await message.reply_text("<b>📸 Please send your payment screenshot now in this chat.</b>\n\nYour request will be forwarded to the admin immediately.", parse_mode=enums.ParseMode.HTML)

    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        return await message.reply_photo(photo=PICS, caption=script.START_TXT.format(message.from_user.mention, temp.U_NAME, temp.B_NAME), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💎 Buy Premium", callback_data="buy_premium_start")]]), parse_mode=enums.ParseMode.HTML)

    if len(message.command) == 2 and any(message.command[1].startswith(p) for p in ["file_", "allfiles_", "BATCH", "DSTORE"]):
        ...
        if user_id not in ADMINS and user_id not in PREMIUM_USER:
            ...
            if col is not None:
                try:
                    ...
                    pass
            is_active = p_doc and isinstance(p_doc.get("expires_at") or p_doc.get("expiry_date"), datetime) and (p_doc.get("expires_at") or p_doc.get("expiry_date")) > datetime.utcnow()
            if not is_active:
                return await message.reply_text("<b>🔒 Premium Required</b>\n\nFile downloading is restricted to <b>Premium Members</b> only.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💎 Buy Premium to Access", callback_data="buy_premium_start")]]), parse_mode=enums.ParseMode.HTML)
...
async def check_my_plan(client, message):
    ...
    if expires_at and isinstance(expires_at, datetime) and expires_at > now:
        ...
        time_left_str = f"{remaining.days}d, {remaining.seconds // 3600}h, {(remaining.seconds % 3600) // 60}m"
...
async def revoke_premium_command(client, message):
    ...
    if result.deleted_count > 0:
        ...
        await log_premium_action(client, f"<b>⚠️ Premium Manually Revoked</b>\n\nTarget User ID: <code>{target_user_id}</code>")
        try:
            await client.send_message(target_user_id, "<b>❌ HeroFlix Premium Revoked</b>\n\nYour membership has been removed by an administrator.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Renew", callback_data="buy_premium_start")]]), parse_mode=enums.ParseMode.HTML)
        except Exception:
            pass
        await message.reply_text(f"<b>Successfully revoked premium for user <code>{target_user_id}</code>.</b>", parse_mode=enums.ParseMode.HTML)
    else:
        await message.reply_text(f"<b>No active premium record found for user <code>{target_user_id}</code>.</b>", parse_mode=enums.ParseMode.HTML)
...
async def list_premiums_command(client, message):
    ...
    if col is None:
        return await message.reply_text("<b>Database collection not found.</b>", parse_mode=enums.ParseMode.HTML)
    now, active_users = datetime.utcnow(), []
    try:
        async for doc in col.find({"active": True}):
            exp = doc.get("expires_at") or doc.get("expiry_date")
    ...
    text = f"<b>💎 Active Premium Users ({len(active_users)})</b>\n\n"
    for idx, doc in enumerate(active_users, 1):
        uid, plan, exp_str = doc.get("user_id"), doc.get("plan", "Standard"), format_date_only(doc.get("expires_at") or doc.get("expiry_date"))
        try:
...
async def generate_invite_command(client, message):
    if len(message.command) < 2:
        return await message.reply_text("<b>Usage:</b> /invite <code>chat_id</code> [expire_hours] [usage_limit]", parse_mode=enums.ParseMode.HTML)
    try:
        ...
        return await message.reply_text("<b>Invalid argument format.</b>", parse_mode=enums.ParseMode.HTML)
    ...
    try:
        ...
        link = await client.create_chat_invite_link(chat_id=target_chat_id, expire_date=(datetime.utcnow() + timedelta(hours=expire_hours)) if expire_hours > 0 else None, member_limit=usage_limit if usage_limit > 0 else None)
        await status_msg.edit_text(f"✅ <b>Invite link generated!</b>\n\n<b>Chat:</b> {chat_obj.title or target_chat_id}\n🔗 {link.invite_link}", parse_mode=enums.ParseMode.HTML, disable_web_page_preview=True)
    except ChatAdminRequired:
        await status_msg.edit_text("❌ <b>Bot lacks Invite Users permission.</b>", parse_mode=enums.ParseMode.HTML)
    except Exception as e:
...
async def minimal_send_proof_cb(client, callback: CallbackQuery):
    MINIMAL_PENDING_FLOW[callback.from_user.id] = {"status": "waiting_screenshot"}
...
async def minimal_admin_action_cb(client, callback: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("1 Month (Test) - ₹40", callback_data=f"selplan_{target_user_id}_30_40"), InlineKeyboardButton("2 Months - ₹80", callback_data=f"selplan_{target_user_id}_60_80")], [InlineKeyboardButton("6 Months - ₹240", callback_data=f"selplan_{target_user_id}_180_240"), InlineKeyboardButton("1 Year - ₹480", callback_data=f"selplan_{target_user_id}_365_480")], [InlineKeyboardButton("❌ Cancel", callback_data=f"min_rej_{target_user_id}")]])
    await callback.answer()
    try:
...
async def select_plan_cb(client, callback: CallbackQuery):
    ...
    is_test = (days == 30 and "Test" in callback.message.text or days == 30)
    plan_label = "1 Month (Test)" if is_test else f"{days} Days"
    now = datetime.utcnow()
    ...
    try:
        if 'db' in globals() and hasattr(db, 'premium_pending'):
            ...
            if not hasattr(client, 'fallback_pending'): client.fallback_pending = {}
...
async def confirm_activation_cb(client, callback: CallbackQuery):
    ...
    if col is not None:
        try:
            existing = await col.find_one({"user_id": target_user_id, "active": True})
        except PyMongoError:
            pass
            
    is_test = (days == 30 and "Test" in plan)
    ...
    if hasattr(client, 'fallback_pending'):
        client.fallback_pending.pop(target_user_id, None)

    activation_data = {"user_id": target_user_id, "username": username, "plan": plan, "plan_days": days, "price": flow.get("price"), "purchased_at": now, "start_date": start_date, "expires_at": expiry_date, "expiry_date": expiry_date, "active": True, "status": "active", "approved_by": callback.from_user.id, "approved_at": now, "welcomed": already_joined, "reminders": {"1_day": False}}
    if col is not None:
        try:
...
async def welcome_premium_user_handler(client, member_update: ChatMemberUpdated):
    ...
    if old_status in [enums.ChatMemberStatus.LEFT, enums.ChatMemberStatus.BANNED] and new_status in [enums.ChatMemberStatus.MEMBER, enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER]:
        ...
        if col is not None:
            ...
            if not user_doc:
                return
            dm_msg_id, plan, exp_str = user_doc.get("dm_msg_id"), user_doc.get("plan", "Standard"), format_date_only(user_doc.get("expires_at") or user_doc.get("expiry_date"))
            perm_link = PREMIUM_PERMANENT_LINK if 'PREMIUM_PERMANENT_LINK' in globals() and PREMIUM_PERMANENT_LINK else "https://t.me/your_group_link"
            ...
            if dm_msg_id:
                try:
                    return await client.edit_message_text(chat_id=user_id, message_id=dm_msg_id, text=joined_text, reply_markup=joined_kb, parse_mode=enums.ParseMode.HTML, disable_web_page_preview=True)
                except Exception:
                    pass
            try:
