...
# 🔑 EDITED: Added RAM Cache dictionary for group settings to eliminate repetitive DB queries
SETTINGS_CACHE = {}
...
# 🔑 EDITED: Cached get_settings function to load from RAM first and fallback to DB
...
# 🔑 EDITED: Cached save_group_settings function to update DB and instantly sync RAM cache
    ...
    async with GLOBAL_SEM:
        ...
        if TUTORIAL:
            tut_url = (
                ...
                else f"https://telegram.me/{TUTORIAL}"
            )
...
async def advantage_spoll_choker(bot, query):
    ...
    async with GLOBAL_SEM:
        ...
        if not movies:
            ...
            try:
                await query.message.edit_text(
                    text="❗Link Expired, Request Again ♻",
                    disable_web_page_preview=True,
                )
            except:
                pass
            ...
            if files:
                ...
                try:
                    msg = await query.message.edit_text(
                        text=script.NO_RESULTS, disable_web_page_preview=True
                    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        if (st.status == enums.ChatMemberStatus.OWNER) or (
            ...
            return await query.answer(
                "Successfully deleted all filters!", show_alert=True
            )
        else:
            return await query.answer(
                script.ALRT_TXT.format(query.from_user.first_name),
        ...
        stat, cb = (
            ("DISCONNECT", "disconnect") if act else ("CONNECT", "connectcb")
        )
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        stat, callback_data=f"{cb}:{group_id}"
                    ),
                    InlineKeyboardButton(
        ...
        if "connectcb" in query.data:
            ...
            msg = (
                f"Cᴏɴɴᴇᴄᴛᴇᴅ ᴛᴏ **{hr.title}**"
                if mkact
                else "Sᴏᴍᴇ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ!!"
            )
        else:
            mkinact = await make_inactive(str(user_id))
            msg = (
                f"Dɪsᴄᴏɴɴᴇᴄᴛᴇᴅ ғʀᴏᴍ **{hr.title}**"
                if mkinact
                else "Sᴏᴍᴇ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ!!"
            )
        ...
        for groupid in groupids:
            try:
                ...
                buttons.append(
                    [
                        InlineKeyboardButton(
                            text=f"{ttl.title}{' - ACTIVE' if active else ''}",
                            callback_data=f"groupcb:{groupid}:{' - ACTIVE' if active else ''}",
                        )
                    ]
                )
            except:
        ...
        reply_text, btn, alerts, fileid = (
            await find_gfilter("gfilters", keyword)
            if "gfilteralert" in query.data
            else await find_filter(query.message.chat.id, keyword)
        )
        if alerts is not None:
            alert = (
                ast.literal_eval(alerts)[int(i)]
                .replace("\\n", "\n")
                .replace("\\t", "\t")
            )
        ...
        if not files_:
            return await query.answer(
                "Nᴏ sᴜᴄʜ ғɪʟᴇ ᴇxɪsᴛ.", show_alert=True
            )
        files = files_[0]
        f_caption = (
            ...
            else (files.caption or files.file_name)
        )
        ...
        try:
            ...
            return await query.answer(
                "Uɴʙʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ ᴍᴀʜɴ !", show_alert=True
            )
        ...
        try:
            if (
                settings.get("botpm", True)
                and settings.get("is_shortlink")
                and clicked not in PREMIUM_USER
            ):
                return await query.answer(
                    url=f"https://telegram.me/{temp.U_NAME}?start=sendfiles1_{key}"
                )
            elif (
                settings.get("is_shortlink")
                and not settings.get("botpm", True)
                and clicked not in PREMIUM_USER
            ):
                return await query.answer(
                    url=f"https://telegram.me/{temp.U_NAME}?start=sendfiles2_{key}"
                )
            else:
                return await query.answer(
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            return await query.answer(
                "😒 You Didn’t Join Channel", show_alert=True
            )
        try:
            ident, file_id = query.data.split("#")
        except:
        ...
        return await query.answer(
            "File sent to PM successfully!", show_alert=True
        )
    elif query.data == "pages":
        return await query.answer(
            "You are on the page navigation.", show_alert=True
        )
    elif query.data.startswith("send_fsall") or query.data.startswith(
        "send_fall"
    ):
        try:
            _, ident, key, offset = query.data.split("#")
        except:
            return await query.answer(
                script.ALRT_TXT.format(query.from_user.first_name),
                show_alert=True,
        ...
        async with lock:
            try:
                for file in files:
                    ...
                    if deleted % 20 == 0:
                        await query.message.edit_text(
                            f"<b>Process started for deleting files from DB. Successfully deleted {deleted} files from DB for your query {keyword} !\n\nPlease wait...</b>"
                        )
            except Exception as e:
                logger.exception(e)
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if (
            ...
            or re.findall("((^\/|^,|^!|^\.|^[\U0001F900-\U000E007F]).*)", message.text)
        ...
        try:
            ...
            btn.append(
                [
                    InlineKeyboardButton("🔅 Page", callback_data="pages"),
                    InlineKeyboardButton(
                        text=f"1/{total_pages}", callback_data="pages"
                    ),
                    InlineKeyboardButton(
...
async def advantage_spell_chok(client, msg):
    ...
    gs = list(
        filter(
            re.compile(r".*(imdb|wikipedia).*", re.IGNORECASE).search, g_s
        )
    )
    gs_parsed = list(
        dict.fromkeys(
            filter(
                ...
                [
                    re.sub(
                        r"\b(imdb|wikipedia|reviews|full|all|episode(s)?|film|movie|tv\s*series|television\s*series|web\s*series|tv\s*show|show|series)\b|[\(\)\-]",
                        " ",
                        i,
                        flags=re.IGNORECASE,
                    ).strip()
    ...
    btn = [
        [
            InlineKeyboardButton(
                text=movie, callback_data=f"spolling#{reqstr1}#{idx}"
            )
        ]
    ...
    k = await msg.reply(
        "<b>🎬 Select Your Pick ↡</b>", reply_markup=InlineKeyboardMarkup(btn)
    )
                ...
                try:
                    ...
                    if fileid == "None":
                        joelkb = await client.send_message(
                            ...
                            protect_content=bool(settings.get("file_secure")),
