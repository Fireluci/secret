...
def get_settings_keyboard(settings, grp_id):
    b = lambda k, t1, t2: InlineKeyboardButton(
        t1 if settings.get(k) else t2, callback_data=f"setgs#{k}#{settings.get(k)}#{grp_id}"
    )
    m = lambda t, k: InlineKeyboardButton(t, callback_data=f"setgs#{k}#{settings.get(k)}#{grp_id}")
    
    return InlineKeyboardMarkup([
        [m('Rᴇsᴜʟᴛ Pᴀɢᴇ', 'button'), b('button', 'Bᴜᴛᴛᴏɴ', 'Tᴇxᴛ')],
        [m('Fɪʟᴇ Sᴇɴᴅ', 'botpm'), b('botpm', 'Mᴀɴᴜᴀʟ', 'Aᴜᴛᴏ')],
        [m('Pʀᴏᴛᴇᴄᴛ', 'file_secure'), b('file_secure', '✔ Oɴ', '✘ Oғғ')],
        [m('Sᴘᴇʟʟ Cʜᴇᴄᴋ', 'spell_check'), b('spell_check', '✔ Oɴ', '✘ Oғғ')],
        [m('Wᴇʟᴄᴏᴍᴇ', 'welcome'), b('welcome', '✔ Oɴ', '✘ Oғғ')],
        [m('Aᴜᴛᴏ-Dᴇʟᴇᴛᴇ', 'auto_delete'), b('auto_delete', '5 Mɪns', '✘ Oғғ')],
        [m('Fɪʟᴛᴇʀ', 'auto_ffilter'), b('auto_ffilter', '✔ Oɴ', '✘ Oғғ')],
        [m('Bᴜᴛᴛᴏɴs', 'max_btn'), b('max_btn', '10', str(MAX_B_TN))],
        [m('SʜᴏʀᴛLɪɴᴋ', 'is_shortlink'), b('is_shortlink', '✔ Oɴ', '✘ Oғғ')]
    ])
...
async def get_search_results_with_darkness(chat_id, query, offset=0, filter=True):
    tokens = [re.escape(word) for word in query.split()]
    if tokens:
        strict_pattern = "".join([rf"(?=.*\b{token}\b)" for token in tokens])
        files, n_offset, total = await get_search_results(chat_id, strict_pattern, offset=offset, filter=True, regex_mode=True)
        if files:
            return files, n_offset, total
    return await get_search_results(chat_id, query, offset=offset, filter=filter)
...
async def next_page(bot, query):
    ...
    async with GLOBAL_SEM:
        ...
        files, n_offset, total = await get_search_results_with_darkness(query.message.chat.id, search, offset=offset, filter=True)
        try:
...
async def advantage_spoll_choker(bot, query):
    ...
    async with GLOBAL_SEM:
        ...
        if await manual_filters(bot, query.message, text=movie) is False:
            files, offset, total_results = await get_search_results_with_darkness(query.message.chat.id, movie, offset=0, filter=True)
            if files:
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        if not search:
            return await query.answer(script.OLD_ALRT_TXT.format(query.from_user.first_name), show_alert=True)
        files, _, _ = await get_search_results_with_darkness(query.message.chat.id, search, offset=int(offset), filter=True)
        ...
        if settings is not None:
            ...
            if query.data.startswith("opnsetgrp"):
                ...
                await query.message.edit_text(text=f"<b>Yᴏᴜʀ sᴇᴛᴛɪɴɢs ᴍᴇɴᴜ ғᴏʀ {title} ʜs ʙᴇᴇɴ sᴇɴᴛ ᴛᴏ ʏᴏᴜʀ PM</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cʜᴇᴄᴋ PM", url=f"telegram.me/{temp.U_NAME}")]]))
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        files, offset, total_results = await get_search_results_with_darkness(message.chat.id, search, offset=0, filter=True)
